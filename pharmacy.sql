...
VALUES (5, '1997-08-18', 'Worker', 21, 'BS', '10000.00'),
    (
        8,
        '1998-06-07',
        'Admin',
...
--
INSERT INTO `employees_audit` (`id`, `Username`, `name`, `changedat`, `action`)
VALUES (
        1,
        'BS',
        'bhavuk',
...
VALUES (1, 'neril', 9878456513, 'new', 'newcom@com.com');
-- --------------------------------------------------------
--
...
VALUES (
        'BS',
        '$2y$10$XsnLoKA07mNjy7spz2pWbeY70r4jZFBuAG6Sb/D6SO3PWwfgL8q7e',
        'Vashi',
        7898546512,
        'bhavuk',
        'Employee'
    ...
    (
        'mohit_15',
        '$2y$10$P6gxhwb.XaUd8Sj3cMkCSuHpI0d.lfRtxlmI66scckBVLCi1GtoTa',
        'kandivali',
        9878456556,
        'mohit',
        'Admin'
    ...
    (
        'mohit_30',
        '$2y$10$ViMNDmXY8bxQ6G91.lQDTefQazz/YDTmhW.MUiIWTps3Dmiv0t8I6',
        'kandivali',
        4565879814,
        'mohit',
        'Admin'
    ...
    (
        'sid_25',
        '$2y$10$RNzm5RRB/k88EW0y0rJJOeYwJ9MUiY4U3f3.tNAB6lF1IdjwIkiiK',
        'borivali',
        7896541336,
        'Siddhant',
        'Employee'
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`localhost` SQL SECURITY DEFINER VIEW `v_medicines` AS
select `stock`.`Med_ID` AS `Med_ID`,
    `stock`.`Batch_no` AS `Batch_no`,
    `stock`.`Med_Name` AS `Med_Name`,
    `stock`.`Quantity` AS `Quantity`,
    `stock`.`Exp_Date` AS `Exp_Date`,
    `stock`.`Category` AS `Category`,
    `stock`.`Selling_Price` AS `Selling_Price`,
    `med_details`.`Mfg_Date` AS `Mfg_Date`,
    `med_details`.`Entry_Date` AS `Entry_Date`,
    `med_details`.`Buying_Price` AS `Buying_Price`,
    `med_details`.`Manufacturer` AS `Manufacturer`
from (
        `stock`
        join `med_details` on(
            (
                (`stock`.`Med_ID` = `med_details`.`Med_ID`)
                and (`stock`.`Batch_no` = `med_details`.`Batch_no`)
            )
        )
    );
--
